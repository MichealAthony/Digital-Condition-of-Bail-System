...
/** UC6 – Update Bail Record. Pre-populates all fields and includes photo upload. */
public class UpdateBailRecordPanel extends JPanel {

    private final BailRecordController controller;
    private final User                  currentUser;
    private final BailRecord            original;
    ...
    public UpdateBailRecordPanel(BailRecordController controller, User currentUser,
                                  BailRecord original, Runnable onSuccess) {
    ...
    private void buildContent() {
        ...
        JLabel t = SwingTheme.makeTitle("Update Bail Record");
        JLabel s = SwingTheme.makeMuted("Editing: " + original.getBailId()
                + "  |  Accused: " + original.getAccusedPerson().getFullName());
    ...
    private void populate() {
        AccusedPerson ap = original.getAccusedPerson();
        fullNameField.setText(ap.getFullName());
        dobField.setText(ap.getDateOfBirth() != null ? ap.getDateOfBirth().toString() : "");
        nationalIdField.setText(ap.getNationalId());
        addressField.setText(ap.getAddress());
        contactField.setText(ap.getContactNumber());
        biometricField.setText(ap.getBiometricRef());
        ap.getOffenses().forEach(offenseModel::addElement);
        original.getConditions().forEach(conditionModel::addElement);
        suretyArea.setText(original.getSuretyInfo());
        // Load existing photo if present
        if (ap.hasPhoto()) photoWidget.loadBase64(ap.getPhotoBase64());
    }
    ...
    private JPanel buildConditionsSection() {
        JPanel card = sectionCard("Bail Conditions *");
        card.setLayout(new BorderLayout(8,8));
        ...
        JScrollPane sp = new JScrollPane(condList);
        ...
        addCond.addActionListener(e -> {
            String desc = condDescField.getText().trim(); if (desc.isEmpty()) return;
    ...
    private JPanel buildActionRow() {
        ...
        row.setOpaque(false);
        JButton cancelBtn = SwingTheme.makeSecondaryButton("Cancel");
        JButton saveBtn   = SwingTheme.makePrimaryButton("Save Changes");
    ...
    private void submit() {
        ...
        catch (DateTimeParseException ex) { dob = null; }

        List<String> offenses = new ArrayList<>();
        for (int i = 0; i < offenseModel.size(); i++) offenses.add(offenseModel.get(i));

        AccusedPerson.Builder ab = AccusedPerson.builder(original.getAccusedPerson().getPersonId())
        ...
        BailRecord result = controller.handleUpdateBailRecord(currentUser, original.getBailId(),
                ab.build(), conditions, suretyArea.getText().trim());
        if (result != null) {
            ...
            statusLabel.setForeground(SwingTheme.ERROR);
        }
    }
